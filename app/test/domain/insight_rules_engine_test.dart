...
import 'package:bodi/domain/models/insight.dart';
...
void main() {
  ...
  test('flags late-night eating after three late meals in a week', () {
    final meals = [
      for (var i = 1; i <= 3; i++)
        meal(DateTime(2026, 7, 20 - i, 22, 30)),
    ];
    final insights = generate(meals: meals);
    expect(insights.map((i) => i.title),
        contains('Late-night eating pattern'));
  });
  ...
  test('celebrates a sustainable downward weight trend', () {
    ...
    final trend = insights
        .firstWhere((i) => i.title == 'Weight is trending down');
  ...
  test('warns on a fast upward weight trend', () {
    ...
    final insights = generate(metrics: metrics);
    final trend =
        insights.firstWhere((i) => i.title == 'Weight is trending up');
  ...
  test('flags high average sodium', () {
    ...
    expect(generate(meals: meals).map((i) => i.title),
  ...
  test('warns when sleep debt builds', () {
    ...
    expect(generate(metrics: metrics).map((i) => i.title),
